...
##################CAMERA INIT##################################
cap = cv2.VideoCapture("/dev/video0") #Catpure video from camera
...
while (1):
        ...
        client.loop_start()
        ###############CAMERA CODE######################################
        _, frame2 = cap.read() #Store Video snap in varialble "frame1"
        ...
        #Red Colour
        low_red = np.array([161,155,84]) #low HSV value for Red objects
        ...
        position_y=int(translate(y_medium, 0, 620, 0, 350)) # 50.0) 
        print("width",w)
        print("hight",h)
        if w > 30 and h > 20:
            print("old",old_position)
            print("new",position_x)
            if old_position<position_x-10:
